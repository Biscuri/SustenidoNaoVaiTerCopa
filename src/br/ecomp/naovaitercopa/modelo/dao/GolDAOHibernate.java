...
public class GolDAOHibernate implements GolDAO {
	...
	/* (non-Javadoc)
	 * @see br.ecomp.naivaitercopa.modelo.GolDAO#adicionar(br.ecomp.naivaitercopa.modelo.Gol)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see br.ecomp.naivaitercopa.modelo.GolDAO#atualizar(br.ecomp.naivaitercopa.modelo.Gol)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see br.ecomp.naivaitercopa.modelo.GolDAO#remover(br.ecomp.naivaitercopa.modelo.Gol)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see br.ecomp.naivaitercopa.modelo.GolDAO#removerTodos()
	 */
	@Override
	public void removerTodos() {
		try {
			...
			Query consulta = (Query) sessao.createQuery("delete from Gol");

			transacao = sessao.beginTransaction();
			((org.hibernate.Query) consulta).executeUpdate();
			transacao.commit();
		} catch (HibernateException e) {
			System.err.println("Nao foi possivel excluir os gols. Erro: " + e.getMessage());
		} finally {
			try {
				sessao.close();
			} catch (Throwable e) {
	...
	/* (non-Javadoc)
	 * @see br.ecomp.naivaitercopa.modelo.GolDAO#listar()
	 */
	@Override
	...
	public List<Gol> listar() {
		...
		try {
			...
			Query consulta = (Query) sessao.createQuery("from Gol");

			transacao = sessao.beginTransaction();
			resultado = (List<Gol>) ((org.hibernate.Query) consulta).list();
			transacao.commit();
			return resultado;
		} 
		catch (HibernateException e) {
			System.err.println("Nao foi possivel listar os gols. Erro: " + e.getMessage());
	...
	/* (non-Javadoc)
	 * @see br.ecomp.naivaitercopa.modelo.GolDAO#buscarGol(java.lang.String)
	 */
	@Override
	public Gol buscarGol(String nome) {
		...
		try {
			...
			Query consulta = (Query) sessao.createQuery("from Gol where nome = :parametro");
			((org.hibernate.Query) consulta).setString("parametro", nome);

			transacao = sessao.beginTransaction();
			gol = (Gol) ((org.hibernate.Query) consulta).uniqueResult();
			transacao.commit();
			return gol;
			
		} catch (HibernateException e) {
			System.err.println("Nao foi possivel buscar o gol. Erro: " + e.getMessage());
		} finally {
			try {
				sessao.close();
			} catch (Throwable e) {
